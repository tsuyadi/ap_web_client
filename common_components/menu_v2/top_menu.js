...
class TopMenu extends React.Component {
	...
	render(){
		let menu = [];
			...
			// console.log('last update ',this.state.lastUpdate);
			// this.state.lastUpdate != "null" ? console.log('a') : console.log('b');
			let last_update = this.state.lastUpdate != null ? new Date(DateFormatExYMDs(this.state.lastUpdate.substring(0,10))) : '-';
			let formated_last_update = '';
			...
			this.jiraHelpdesk();
			if(localStorage.getItem('role') == 17 || localStorage.getItem('role') == 18 || localStorage.getItem('role') == 19 || localStorage.getItem('role') == 20){
				menu.push(
					<ul> 
						...
						<div className="afterBottom">
							<div className="left">
								<ul className="nav nav-tabs" role="tablist">
									...
									{localStorage.getItem('role') != 18 || localStorage.getItem('role') != 19 || localStorage.getItem('role') != 20 ? <li role="presentation"><a href="#groupselling" aria-controls="groupselling" role="tab" data-toggle="tab" id="tabgroup">Group Selling</a></li> : ''}
								</ul>
							</div>
							<div className="left">
                                <div className="lastupdatelabel">Last Update : {this.state.lastUpdate != "null" ? formated_last_update : '-'} </div>
							</div>
							<div className="right">
