  ...
  componentDidMount(){
	...
	getData (claim_id) {
		...
		$.ajax({
            ...
            type: 'POST',
			data: {claim_id: claim_id},
            success: (response_claim) => {
              $('#loading').modal('hide');

			this.setState({
					data_claim: response_claim.content
              });
            },
			error: (xhr, status) => {
				$('#loading').modal('hide');
	...
	// }
	render(){
	let data = this.state.data && this.state.data.document_serializer;
    ...
    if(data != null && data.length > 0){
		...
		$.map(notes, (value, index) => {
			document.push(
				<tr key={index}>
					...
					<td>{value.type}</td>
					<td>{DateFormat(value.upload_date)}</td>
					<td>{DateFormat(value.ho_receive)}</td>
					<td>{value.status}</td>
				</tr>
			);
		});
		...
		<div className="wrap2">
			<SubmitModal />
			<FeatureModal />
            <TopMenuNewBusinessDetail opsi="claim" title="Document" id={this.props.params.claim_id} />
												...
												<table className="table table-bordered table-striped table-hover table-box">
													<thead>
														<tr>
															...
															<th className="header_table ">Upload Date</th>
															<th className="header_table ">HO Receive</th>
