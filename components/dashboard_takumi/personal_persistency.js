...
class  PersonalPersistency extends React.Component {
	constructor(props){
		super(props);
		this.state = {
			data : null,
	...
	handleShowModal(p){
		if(p == 1 )
		{
			let ecp = this.state.data && this.state.data.takumi_dashboard_personal.ecp_p1;
			let acp = this.state.data && this.state.data.takumi_dashboard_personal.acp_p1;
			this.setState({
		...
		{
			let ecp = this.state.data && this.state.data.takumi_dashboard_personal.ecp_p2;
			let acp = this.state.data && this.state.data.takumi_dashboard_personal.acp_p2;
			this.setState({
	...
	render(){
		let ecp_p1 = this.state.data && this.state.data.takumi_dashboard_personal.ecp_p1 != null ? this.state.data.takumi_dashboard_personal.ecp_p1 : 0;
		let acp_p1 = this.state.data && this.state.data.takumi_dashboard_personal.acp_p1 != null ? this.state.data.takumi_dashboard_personal.acp_p1 : 0;
		ecp_p1 = ecp_p1 != 0 ? MoneyFormat(ecp_p1) :0;
		acp_p1 =  acp_p1 != 0 ? MoneyFormat(acp_p1) :0;

		let ecp_p2 = this.state.data && this.state.data.takumi_dashboard_personal.ecp_p2 != null ? this.state.data.takumi_dashboard_personal.ecp_p2 : 0;
		let acp_p2 = this.state.data && this.state.data.takumi_dashboard_personal.acp_p2 != null ? this.state.data.takumi_dashboard_personal.acp_p2 : 0;
		ecp_p2 = ecp_p2 != 0 ? MoneyFormat(ecp_p2) :0;
		acp_p2 = acp_p2 != 0 ? MoneyFormat(acp_p2) :0;
		
		var pg1 = ecp_p1 == 0 && acp_p1 == 0 ? 100 : this.state.data && this.state.data.takumi_dashboard_personal.persistence_p1;
		var pg2 = ecp_p2 == 0 && acp_p2 == 0 ? 100 : this.state.data && this.state.data.takumi_dashboard_personal.persistence_p2;
        ...
        <div className="content boxShadow">
			...
			<div className="modal fade" id="personal_persistency" tabIndex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
export default PersonalPersistency;