...
class SamReport extends React.Component {
	constructor(props){
		super(props);
		CekAuth();
		this.state = {
	...
	handleReportDownload(e){
		...
		if(this.state.reportType == 'personal'){
			...
			//var url = api_route.production_report_agentpersonal + this.state.agent_id+ (this.state.reportYear != 2018 ? '?year='+this.state.reportYear : '')+(localStorage.role == '9' ? '' : (this.state.reportYear == 2018 ? '?' : '&')+'detail=1');
			var url = api_route.production_report_agentpersonal + this.state.agent_id+ (this.state.reportYear != 2018 ? '?year='+this.state.reportYear : '')+(localStorage.role == '9' || localStorage.role == '8' || localStorage.role == '14' ? (this.state.reportYear == 2018 ? '?token='+sessionStorage.getItem('token') : token) : (this.state.reportYear == 2018 ? '?' : '&')+'detail=1'+token);
			window.location.href=url;
			console.log('url : '+url);
		}else if(this.state.reportType == 'group'){
			var url = api_route.production_report_agent + this.state.agent_id+ (this.state.reportYear != 2018 ? '?year='+this.state.reportYear+'&detail=1' : '?detail=1');
	...
	render(){
		...
		if(localStorage && localStorage.role == "5")
		{
			...
			tablist_name.push(
				<ul className="nav nav-tabs" role="tablist">
					<li role="presentation" className="active"><a href="#comm_report" aria-controls="comm_report" role="tab" data-toggle="tab">Open Case Selling</a></li>
					<li role="presentation"><a href="#prod_report_hier" aria-controls="prod_report_hier" role="tab" data-toggle="tab">Open Case Recruit</a></li>
					<li role="presentation"><a href="#prod_report_loc" aria-controls="prod_report_loc" role="tab" data-toggle="tab">Detail Activity Inquiry</a></li>
				</ul>
			);
